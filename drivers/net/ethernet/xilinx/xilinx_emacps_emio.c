...
#define DRIVER_NAME			"xemacps_emio"
...
static void poll_gmii(unsigned long data)
{
  ...
  if (get_phy_status(dev, &phy_duplex, &phy_carrier)) {
    printk(KERN_ERR "%s: emacps_emio: terminating link monitoring.\n",
           dev->name);
    return;
  }
  ...
  if (phy_carrier != netif_carrier) {
    if (phy_carrier) {
      printk(KERN_INFO
             "%s: emacps_emio: PHY Link carrier restored.\n",
             dev->name);
      netif_carrier_on(dev);
//                set_mac_speed(lp);
    }
    else {
      printk(KERN_INFO "%s: emacps_emio: PHY Link carrier lost.\n",
             dev->name);
...
static int xemacps_mdio_read(struct net_local *lp, int mii_id, int phyreg)
{
...
//	u16 value)
static int xemacps_mdio_write(struct net_local *lp, int mii_id, int phyreg,
	u16 value)
{
	//struct net_local *lp = bus->priv;
	u32 regval;
...
static int get_phy_status(struct net_device *dev, DUPLEX * duplex, int *linkup)
{
  ...
  reg1 = xemacps_mdio_read(lp, lp->gmii_addr, MII_BMCR);
  *duplex = FULL_DUPLEX;

  reg2 = xemacps_mdio_read(lp, lp->gmii_addr, MII_BMSR);
...
static int xemacps_mii_init(struct net_local *lp)
{
  ...
  Register = xemacps_mdio_read(lp, 6, 0);
  printk(KERN_ERR "1000BASE-X: PHY Control Reg = %x\n", Register);
  // - Reset core
  xemacps_mdio_write(lp, lp->gmii_addr, 0, (Register | 0x8000));
  // - Clear Isolate bit for normal operation
  xemacps_mdio_write(lp, lp->gmii_addr, 0, (Register & 0xFBFF));
  Register = xemacps_mdio_read(lp, lp->gmii_addr, 0);
  printk(KERN_ERR "1000BASE-X: PHY Control Reg = %x\n", Register);
...
static int xemacps_probe(struct platform_device *pdev)
{
  ...
  ndev->features = NETIF_F_FRAGLIST;
...
#ifdef USE_EMIO_RST
	printk(KERN_ERR "%s :: EMIO RESET \n",__func__);
	lp->gpio_rst = devm_gpiod_get(&lp->pdev->dev, "gt-reset");
   ...
   gpiod_direction_output(lp->gpio_rst, 0);
	...
	if (!IS_ERR(lp->gpio_rst))
		gpiod_set_value_cansleep(lp->gpio_rst, 1);
#endif
...
static struct platform_driver xemacps_emio_driver = {
	.probe   = xemacps_probe,
...
static int __init xemacps_emio_init(void)
{
  platform_driver_register(&xemacps_emio_driver);
  return 0;
}
static void __exit xemacps_emio_exit(void)
{
  platform_driver_unregister(&xemacps_emio_driver);
}

module_init(xemacps_emio_init);
module_exit(xemacps_emio_exit);
